...
#    - Fedora / CentOS / Alpine (best-effort)
#
...
if   [ -n "${CODESPACES:-}" ];           then print_info "Detected: GitHub Codespaces"
elif [ -n "${GITPOD_WORKSPACE_ID:-}" ];  then print_info "Detected: Gitpod"
elif grep -qi microsoft /proc/version 2>/dev/null; then print_info "Detected: WSL"
else                                          print_info "Detected: Linux"
fi
...
if [ -z "$USE_MODE" ] && [ -d "$INSTALL_DIR" ]; then
    ...
    if [ -n "$HAS_LOCAL_ID" ]; then
        echo -e "${BOLD}Existing Billions identity found on this machine:${NC}"
        echo "    $HAS_LOCAL_ID"
        echo ""
...
USE_SUDO=""
command -v sudo &>/dev/null && USE_SUDO="sudo"
...
if command -v node &>/dev/null; then
    ...
    command -v node &>/dev/null || { print_error "Node.js installation failed."; exit 1; }
...
if command -v git &>/dev/null; then
    ...
    if   command -v apt-get &>/dev/null; then $USE_SUDO apt-get update && $USE_SUDO apt-get install -y git
    elif command -v dnf &>/dev/null;     then $USE_SUDO dnf install -y git
    elif command -v yum &>/dev/null;     then $USE_SUDO yum install -y git
    elif command -v apk &>/dev/null;     then $USE_SUDO apk add --no-cache git
    else print_error "Could not detect package manager. Install Git manually."; exit 1; fi
...
if npx --yes clawhub@latest install verified-agent-identity 2>&1; then
    ...
    [ ! -f "package.json" ] && npm init -y 2>&1 && print_info "Created package.json."
fi
